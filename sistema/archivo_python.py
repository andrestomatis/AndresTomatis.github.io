...
url = f'https://eightroom.voonix.net/api/?report=advertiserearnings&list&key=c2fb5fb9293a5ebe1c83ae8d53b1ca5773456fd9&start=2023-06-01&end=2023-06-30'
...
def modify_json(json_data):
    ...
    modified_data = []
    key_counter = 1  # Contador para el valor de 'key'
                ...
                for campaign_id, campaign_data in login_data.items():
                    for date, date_data in campaign_data.items():
                        item = {
                            ...
                            "date": date,
                            "key": str(key_counter) + "_" + date,  # Agregar 'key'
                            **date_data
                        }
                        modified_data.append(item)
                        key_counter += 1  # Incrementar el contador de 'key'
